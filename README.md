...
- `npm run links` – builds the Jekyll site and then runs a linkcheck using [lychee](https://github.com/lycheeverse/lychee) against generated HTML in `_site/`. This catches URL behavior as deployed and honors exclusions listed in `lychee.toml`.

  If `lychee` is not installed locally, install it separately (e.g. `cargo install lychee` or download a prebuilt binary); CI uses the official `lycheeverse/lychee-action`.
...
- External URLs that are known to block bots or are intentionally flaky may be added to the `exclude` list in `lychee.toml`. Provide a brief comment when you add an entry explaining why it’s excluded.
...
2. **links.yml** – also runs on `push` and `pull_request`. In CI it builds the Jekyll site with Bundler, then invokes the `lycheeverse/lychee-action` against `_site/**/*.html`; a failing linkcheck will block merges to the protected branch.
...
- Node tooling is used for spelling and local task orchestration; the site build itself remains Jekyll/Bundler-based.
